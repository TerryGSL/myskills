# Harness Rules for Cursor

> Cursor 自动加载本目录（`.cursor/rules/`）下的所有 `*.md` 作为持久化规则，
> 每次新对话注入到 system prompt。本文件是 Harness 工作流在 Cursor 下的入口，
> 包含完整 7 条 kernel + CLI surface + mandatory contracts。
>
> **跨工具入口的关系**：与 `AGENTS.md`（Codex / Cross-tool）、
> `harness-init/SKILL.md`（Claude Code）、`CONVENTIONS.md`（Aider）、
...
- **自动加载机制**：Cursor 启动会扫描项目根 `.cursor/rules/*.md`，把内容拼进
  system prompt。这是 Cursor 自带能力，无需额外配置；新增/修改本目录文件后
  重启 Cursor chat 生效。
- **Cursor chat history**：Cursor IDE 自带的对话历史是工具自带 session memory
  （单工具有效），按 kernel 第 7 条 + Project Memory 段处理 —— 跨会话/跨工具
  仍须以 `docs/memory/*.md` 为准。
- **多文件规则**：可在 `.cursor/rules/` 下加更多 `*.md`（如项目专属约定），
  Cursor 会全部加载；本文件只覆盖 Harness 通用 kernel。
- **缺 CLI 时**：Cursor 终端可跑 `harness install --doctor` 探测；如果无 node
  环境，按 kernel #4 的 Tier 3 markdown fallback 手算 7-field route object。
