...
#   - Adaptive thresholds by effective_task_type:
#       quick           → warn=80, crit=90
#       bugfix          → warn=70, crit=85
#       feature/refactor→ warn=60, crit=80
#       (default / 无)  → warn=70, crit=85
#   - Each threshold can be overridden via HARNESS_*_WARN_THRESHOLD /
#     HARNESS_*_CRIT_THRESHOLD env vars.
#
# SCRIPT_PATH: $REPO_ROOT/hooks/context-monitor.sh
#   harness/hooks/context-monitor.sh is a symlink → ../../hooks/context-monitor.sh
#   New users register the top-level path in ~/.claude/settings.json.
#   Existing users registered to harness/hooks/ keep working via the symlink.
#
...
if (( pct >= threshold_crit )); then
  ...
  echo -e "  [ ] 项目级 memory     — 关键决策/发现是否已写入 docs/memory/* 或 .harness/learnings/*（必需）"
  echo -e "  [ ] 工具自带加速器     — claude-mem observation / codex resume / cursor history（可选）"
...
#           {
#             "type": "command",
#             "command": "/Users/twelve/Music/myskills/hooks/context-monitor.sh"
#           }
#         ]
#       }
#     ]
