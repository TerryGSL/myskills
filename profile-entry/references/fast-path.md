...
Single source of truth for the fast-path decision logic that auto-routes trivial diffs to `harness-quick` without asking the user.
...
**核心原则（Deterministic）**：基于 `git diff --stat` + 文件路径 pattern + per-language 正则，**不是 LLM 猜测**。相同 diff 每次判定一致；用确定性规则替代 LLM 猜，永不调用语言模型做任务类型判断。
...
### 条件 1：无显式任务类型 flag
用户消息中不含 `/quick`、`/fix`、`/refactor`、`/feature` 任一。
若含 flag，直接按 flag 解析，跳过 fast-path。

### 条件 2：`git diff --stat` 仅 1 文件改动
```bash
...
扩展名 ∈ `{.md, .txt, .json, .yml, .yaml, .toml}` → 直接过（除非触发下面 Category C 的 exclusion）

这些文件不含可执行代码，结构变更风险极低。

**Exception**：即使扩展名对，若文件名匹配 Category C → 不过。

### Category B：源码文件（需扫 diff 内容）

扩展名 ∈ `{.ts, .js, .tsx, .jsx, .py, .java, .go, .rs, .kt, .scala}` → 需要 diff **不包含**以下任一类别：
...
- `migrations/` 目录下任何文件，路径含 `migration` / `migrate` / `schema`
- `*migration*.sql` / `V*__*.sql`（Flyway migration）
- `schema.sql` / `*.schema.json` / `**/*.schema.*`（API schema 定义）
...
**不用 AST 的理由**：速度 + 跨语言一致性。Regex 足够排除"签名变化"这种粗粒度信号。
...
### 误判 / FP（结构变更漏进 quick-path）

**风险等级**：**严重** —— quick 不过 strict-reviewer，坏改动直接 commit。
...
4. **Category C 黑名单兜底**：依赖、migration、CI、Dockerfile 永远不走 fast-path。
5. **文档化缓解**：本文件明确列出已知盲点（如同一函数体内的副作用变更），便于后续调整 allowlist。
