# Harness Conventions for Aider

> Aider 的 `--read` 机制把本文件作为只读上下文注入每次对话。本仓库的
> `.aider.conf.yml` 已配置 `read: [CONVENTIONS.md, harness-common/contracts/routing.md]`，
> Aider 启动后自动加载，无需手动 `/read`。
>
> **跨工具入口的关系**：与 `AGENTS.md`（Codex / Cross-tool）、
> `harness-init/SKILL.md`（Claude Code）、`.cursor/rules/harness.md`（Cursor）、
> `.github/copilot-instructions.md`（Copilot）严格一致。
...
- **配置入口**：`.aider.conf.yml` 已声明 `read: [CONVENTIONS.md, harness-common/contracts/routing.md]`，
  Aider 在仓库根启动时会自动 load。也可手工 `aider --read CONVENTIONS.md` 显式加载。
- **跨会话恢复**：Aider 自带 `--restore-chat-history`（基于 `.aider.chat.history.md`）
  是工具自带 session memory（仅 Aider 单工具内有效）；跨工具协作仍须以
  `docs/memory/*.md` 为准。
- **新会话推荐启动姿势**：
  ```bash
  aider --restore-chat-history       # 续上次会话（可选）
  aider --read CONVENTIONS.md ...    # 默认已经在 .aider.conf.yml 里，无需重复
  ```
- **缺 CLI 时**：Aider 可在 shell 内调 `harness install --doctor`；无 node 环境时
  按 kernel #4 的 Tier 3 markdown fallback 手算 7-field route object。
- **commit 前**：依赖 kernel #6 refusal 与 #7 routing handoff，不要让 Aider 直接
  `--auto-commit` 跳过 hard_floor / push-check。
