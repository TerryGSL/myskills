...
review_target:
  ...
  knowledge_snapshot_id: null   # null | "scan-..." — 本次 knowledge scan 的唯一 ID
  relevant_knowledge_files: null  # null | ["docs/harness/knowledge/.../manifest.md", ...]
  knowledge_requirements: null  # null | [{rule_id, manifest_file, applies_to, requirement_text, violation_test, <extra fields>}]
  retrieval_outcome: null       # null | "success" | "coordinator_miss" | "all_candidates_filtered"
  known_issues: null            # null | [{source, id, domain, reason}]
```
...
5. **Knowledge Compliance Check**（仅当 `knowledge_requirements != null`）
   - 对 `knowledge_requirements` 每一条 Rule：
     - `violation_test` 是结构化 enum（must_use_wrapper / must_call_component 等）→ 用 rg / grep / symbol search 对 diff 做机器可验证检查
     - `violation_test = free_form_review` → 读 Rule 的 `requirement_text`，人工判断 diff 是否符合
     - 违反 → 加 finding: severity="high", grounded_by="read:file", message="violated knowledge rule: <rule_id> — <requirement_text>"
   - `known_issues` 数组：**不作为 FAIL 依据**，但在 scorecard 里记录（供 audit / false-pass correction 追溯）
...
| `retrieval_outcome = "all_candidates_filtered"` | 不 BLOCK — 本次跳过 knowledge-grounding，继续 adversarial + 三硬门；scorecard 记 `known_issues_echoed` |
| INDEX 存在但 `relevant_knowledge_files = null` | **BLOCKED** |
