...
## Cross-Tool Adapter Map

| Tool        | Wrapper                                  | Status  |
|-------------|------------------------------------------|---------|
| Claude Code | `harness-init/SKILL.md` (Skill tool)     | Tier-1  |
| Codex       | `AGENTS.md` (this file)                  | Tier-1  |
| Cursor      | `.cursor/rules/harness.md`               | Tier-2  |
| Aider       | `CONVENTIONS.md` + `.aider.conf.yml`     | Tier-2  |
| Copilot     | `.github/copilot-instructions.md`        | Tier-2  |

Tier-1：内置完整 7 kernel + duplicated detail（本文件 / harness-init/SKILL.md）。
Tier-2：薄壳指向 Tier-1 + contracts，不复制 kernel 全文。
