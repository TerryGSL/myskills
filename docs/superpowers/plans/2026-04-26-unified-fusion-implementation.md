...
**Architecture:** 5 个 Phase（A1 → A2 → B / C 并行 → D → E），跨 Phase 必须串行；每个 Phase 内可并行。每 PR atomic + rollback。
...
**Hard Constraints**:
- 跨 Phase 严格串行，跨 PR 必须 commit message 带 `[depends on: PR N-1]` tag
...
4. 跑 jest（应仍 107 PASS，因为这只是数组扩张和文件重命名）
5. Commit message: `chore(drift): align hard_floor flags 4→6, marker format YAML, SKILL.md case [PR A1]`

**Rollback:** `git revert <pr-a1-sha>`。
...
**Rollback:** `git revert <pr-a2.1-sha>`。
...
**Rollback:** `git revert <pr-a2.2-sha>`。
...
2. 把规则展开内容迁到对应 contracts/*.md（如 push-decision 规则 → contracts/push-decision.md，已迁则确认引用）
...
**Files:**
- Create: `packages/harness-cli/src/commands/scan.ts`
...
- Modify: `packages/harness-cli/src/commands/scan.ts`（输出含 knowledgeCheck）
...
3. Commit: `feat(retrieval): Stage -0.5 with 8-field knowledgeCheck [PR C3] [depends on: PR C2]`
...
5. Commit: `feat(route): unified routing CLI command [PR C5] [depends on: PR C4]`
...
4. Commit: `refactor(profile-entry): Tier-3 fallback role + leaf SKILL input contract [PR C6] [depends on: PR C5]`
...
3. Commit: `refactor(memory): invert claude-mem dependency to optional [PR E1] [depends on: PR D2]`

### PR E2：删除冗余 + 文档同步

**Files:**
- 删除：现状下重复的 SKILL 旧版本（identifier by content drift after PR B3 / C6）
...
跨 Phase 严格串行；Phase 内多 PR 顺序。每 PR commit message 含 `[depends on: PR X]` tag。

总计：14 PR / ~20 新文件 / ~30 修改文件 / 估计 ~60 commits。
