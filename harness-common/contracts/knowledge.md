...
> 本节描述 scanner subagent **探测时的内部维度**（spec 1 设计），不是 init 预塞
> 到磁盘的目录列表。`harness init` 仅创建 `_example/` 通用骨架，不预创建 domain
> 子目录；`harness scan` 跑 detector 后 manifest 只含真实有 evidence 的 domain。

| Domain | 触发激活 | 内容 |
|--------|---------|------|
| `style-and-structure` | scout 探测命名 / 模块（项目通常激活）| 命名 / 模块组织 / 文件布局 |
| `internal-components` | scout 探测内部 wrapper（项目通常激活）| 内部 wrapper / DI / helper |
...
每个 domain 是否激活由 scout 信号决定；探测无证据 → 不 emit。

> 已知 spec 漂移：本表 5-domain（business 维度）与 `packages/harness-cli/src/
> utils/knowledge-scanner.ts` 的 5 detector（`api / db / business-rules /
> config / deployment` 工程维度）**是两套不同的 5-domain**，待 spec 1 v2 统一。
